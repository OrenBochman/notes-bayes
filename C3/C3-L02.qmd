...
$$
\mathbf{x}_i \sim f \qquad f(\mathbf{x} ) = \sum_{k=1}^K w_k \cdot g_k(x_i \mid \mathbf{\theta_k})
...
- where:
  ...
  - $\mathbf{\theta}$ are the parameters of the components.
...
$$
\mathcal{L}(\mathbf{\omega},\mathbf{\theta} \mid  \mathbf{x}) = \prod_{i=1}^n \sum_{k=1}^K w_k \cdot g_k(x_i \mid \theta_k)
...
- where 
  ...
  - $\mathbf{\omega}$ are the weights of the mixture components.
  - $\mathbf{\theta_k}$ are the parameters of the components.
...
$$
x_i \mid C_i \sim g_{C_i}(x_i \mid \theta_{C_i}) \qquad \mathbb{P}r(C_i=k) = w_k \qquad C_i \stackrel{i.i.d.}{\sim} \text{Categorical}(\mathbf{\omega})
$$ 
...
$$
\begin{aligned}
\mathcal{L}(\mathbb{\omega},\mathbb{\theta}, \mathbf{C}) &= \prod_{i=1}^n \prod_{k=1}^K [\omega_k g_k(x_i)]^{\mathbb{I}_{(C_i=k)}}  \\
&=
...
- The right hand term is the marginal likelihood of the latent variables: $p(C_i \mid \mathbf{\omega})$.
