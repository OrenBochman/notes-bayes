...
::: {#exr-ANOVA-1}
[ANOVA]{.colum-margin} Which of the following variables qualifies as a "factor" variable?
:::
...
::: {#exr-ANOVA-4}
[ANOVA]{.colum-margin} Compute the deviance information criterion (DIC) for each of the two models and save the results as objects dic1 (for the original model) and dic2 (for the new model). Wha is the difference: DIC1 - DIC2?.
...
::: {#exr-ANOVA-5}
[ANOVA]{.colum-margin} Based on the DIC calculations for these competing models, what should we conclude?
:::
...
::: {#exr-ANOVA-6}
[ANOVA]{.colum-margin} Use the original model (single variance) to calculate a 95% interval of highest posterior density (HPD) for $\mu_3−\mu_1$. Which of the following is closest to this interval?
:::
...
::: {#exr-ANOVA-7}
[ANOVA]{.colum-margin} What is the correct interpretation of $\mu_3−\mu_1$ in the context of the plant growth analysis?
:::
...
::: {#exr-ANOVA-8}
[ANOVA]{.colum-margin} The linear model with a baseline mean and group effects is the default in R. However, we can also fit the cell means model in R using the following code:
