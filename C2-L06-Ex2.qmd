...
[M-H]{.colum-margin} For Questions @exr-mhh-1 through @exr-mhh-3, consider the following model for data that take on values between 0 and 1:
...
::: {#exr-mhh-1}
[M-H]{.colum-margin} Which of the following gives the *full conditional density* for $α$ up to proportionality?
:::
...
::: {#exr-mhh-2}
[M-H]{.colum-margin} Suppose we want posterior samples for α from the model in @exr-mhh-1. What is our best option?
:::
...
::: {#exr-mhh-3}
[M-H]{.colum-margin} If we elect to use a Metropolis-Hastings algorithm to draw posterior samples for $α$, the Metropolis-Hastings candidate acceptance ratio is computed using the full conditional for $α$ as
...
::: {#exr-mhh-4}
[M-H]{.colum-margin} Below are four possible values for the standard deviation of the normal proposal distribution in the algorithm. Which one yields the best sampling results?
:::
...
::: {#exr-mhh-5}
[M-H]{.colum-margin} Report the posterior mean point estimate for μ, the mean growth, using these six data points. Round your answer to two decimal places.
:::
