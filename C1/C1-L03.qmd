...
$$
f(X=x\mid p) = p^x(1-p)^{1-x} \mathbb{I}_{[0,1]}(x)
$$ {#eq-l3-bernoulli-pmf}
...
```{mermaid}
%%| label: fig-binomial-mindmap
%%| column: body-outset-right
%%| fig-width: 8
%%| fig-cap: |
%%|   Binomial distribution mindmap
---
config:
  layout: tidy-tree
---
mindmap
  ...
  Distribution))
    Parametrizations
      Standard["$$p \in [0, 1]$$"]
        Mean_p["$$\mathbb{E}[X] = np$$"]
        Var_p["$$\text{Var}(X) = np(1-p)$$"]
      Natural_Alpha["$$\alpha = \ln\left(\frac{p}{1-p}\right)$$"]
        Space["$$\alpha \in \mathbb{R}$$"]
        Logistic["$$p = \frac{1}{1 + e^{-\alpha}}$$"]
    (Information 
     Geometry)
      Fisher_Info["$$\mathcal{I}(p) = \frac{n}{p(1-p)}$$"]
      KL_Divergence["$$D_{KL}(P || Q)$$"]
      Entropy["$$H(X) \approx \frac{1}{2}\log(2\pi e npq)$$"]
    (Relationships)
      Discrete_Limits
        Bernoulli["$$\mathcal{Bin}(1, p) \equiv \mathcal{Bern}(p)$$"]
        Poisson["$$\lim_{n \to \infty, np=\mu} \mathcal{Bin} \to \mathcal{Pois}(\mu)$$"]
      Continuous_Limits
        Normal["$$\lim_{n \to \infty} \mathcal{Bin} \approx \mathcal{N}(np, npq)$$"]
        BetaBinomial["$$p \sim \text{Beta}(a, b)$$"]
      Sampling_Limits
        Hypergeo["$$\lim_{n_3 \to \infty} \text{Hyper} \to \mathcal{Bin}$$"]
        Polya["$$\lim_{\beta \to 0} \text{Polya} \to \mathcal{Bin}$$"]
    Assumptions
      n_Fixed["n Fixed trials"]
      Indep["Independent trials"]
