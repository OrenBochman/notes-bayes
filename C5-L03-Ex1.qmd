...
# Homework - Determine number of components for the mixture model -- M3L_3 {#sec-capstone-lm-infer-mix-comp}

1. Using the function to calculate DIC score for different number of components. 
Provide the DIC when using K = 2, 3, 4, 5 components. 
Based on your calculation, what is the number of mixing component you will use for earthquake data?
...
1. Using the above function to calculate DIC score for different number of components. 
Provide the DIC when using K = 2, 3, 4, 5 components. 
Based on your calculation, what is the number of mixing component you will use for earthquake data?
...
model_comp_mix=function(tot_num_comp){
  ...
  sample_L_one=function(beta.cur,omega.cur,nu.cur,y.cur,Fmtx.cur){
    prob_k=function(k){
      beta.use=beta.cur[((k-1)*p+1):(k*p)]
      omega.cur[k]*dnorm(y.cur,mean=sum(beta.use*Fmtx.cur),sd=sqrt(nu.cur) + 1e-10)
    }
    prob.vec=sapply(1:K, prob_k)
    L.sample = sample(1:K, 1, prob = (prob.vec + 1e-8) / sum(prob.vec + 1e-8)) 
...
2. Using the above function to calculate DIC score for different number of components. 
Calculate the DIC when using K = 2, 3, 4, 5 components. 
Based on your calculation, what is the order you will use for Google search index data?
