...
:::: {#exr-pfw-2-7-7}
7. Consider the AR(2) series $y_t = \phi_1 y_{t−1} + \phi_2 y_{t−2} + \varepsilon_t$ with $\varepsilon_t \sim \mathcal{N}(0, \nu)$.
...
forecast function is $E(y_{t+k} \mid \boldsymbol{\Lambda}_t ) = \mathbf{F}^\top \mathbf{G}^k \mathbf{x}_t$ .
    ...
    $$
    \begin{align}
    \boldsymbol{\Lambda} ^k \mathbb{E}^{-1} =\frac{1}{\lambda_1-\lambda_2}\begin{pmatrix}
    ...
    $\mathbb{E}(y_{t+k} \mid x_t ) = a_k y_t + b_k y_{t−1}$ gives
    ...
    $\mathbb{E}(y_{t+k} \mid x_t) = r^k h_t \cos(k\omega + g_t ), \quad k = 0, 1, \ldots,$
...
:::: {#exr-pfw-2-7-10}
10. Show that if an $AR(2)$ process has a pair of complex roots given by $r\sqrt{\exp(\pm i\omega)}$, they can be written in terms of the AR coefficients as $r =\sqrt{- \phi_2}$ and $cos(\omega) = \phi_1 /2r$.
::::

:::: {#exr-pfw-2-7-11}
11. Plot the corresponding forecast functions for the $AR(2)$ processes considered in Example 2.1.
::::

:::: {#exr-pfw-2-7-12}
12. Verify that the expressions for the conditional posterior distributions in [@prado2023time § 2.4.1] are correct.
::::

:::: {#exr-pfw-2-7-13}
13. Show that a prior on the vector of $AR(p)$ coefficients $\phi$ of the form
$\mathcal{N} (\phi_1 |0, w/\delta_1 )$ and $\mathcal{N} (\phi_j \mid \phi_{j−1} , w/ \delta_j)$ for $1 < j ≤ p$ can be written as $p(\phi) = \mathcal{N} (\phi \mid 0, A^{−1} w)$, where $A = H^\top \Delta H$ with $H$ and $\Delta$ defined in
[@prado2023time § 2.4.2].
::::
...
:::: {#exr-pfw-2-7-17}
17. Consider the $AR(1)$ model given by $(1 − \phi B)(y_t − µ) = \varepsilon_t$
where $\varepsilon_t \sim \mathcal{N} (0, \nu)$.
    (a) Find the MLEs for $\phi$ and µ when $µ \ne 0$.
    (b) Assume that $v$ is known, $µ=0$, and that the prior distribution 
    for $\phi$ is $U(\phi\mid 0,1)$. 
    Find an expression for the posterior distribution of \phi.
::::
