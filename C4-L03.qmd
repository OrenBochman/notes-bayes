...
φ_{t,m} &= φ_{t−1,m} + \omega_{t,m,} & m = 1 : p
\end{aligned}
$$ {#eq-tvar}
...
### Reality check {.unumbered}


The next bit tells us what we know and then what we want to use baysian inference for (i.e. filtering smoothing, of the parameters $\theta_t$ given the data and forecasting $y_t$ based on that)
...
However, at some point I saw a talk about [STUMPY](https://stumpy.readthedocs.io/en/latest/Tutorial_STUMPY_Basics.html) which does many cool time series stuff in python. And the speaker talks about all the different data science and DSP 'magic' one can use, but also about the problems comparing lots of times series or time series with lots of data. 

Prado kind of suggests that a time series has around 300 data points. And the DLM framework while flexible assumes that you will know the actual form of the model and priors. I suppose they expect that you will use some other non-bayesian tools to do this as we haven't really covered this in her course. 
...
Here $\mathcal{D}_0$ stands for the information that I have before collecting any data. And we are assuming $\theta_0$ follows a normal distribution with $m_0$ mean and variance covariance matrix $C_0$. So these are also specified when you're working with this model. 
