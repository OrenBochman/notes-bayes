...
- This section is a labour of love. 
- Its should read like "Everything you wanted to know about NDLMs but were afraid to ask" meets "Everything I should have known about NDLM twenty years ago, when I was getting started." 
- It arose out of skimming thought the books which were too long and hard to read initially. The book seem to have all the answers but you really must know the questions and then read very very carefully. The books have different issues. 
- It tries to **connect the dots** that were not connected in class. 
- It also **adds dots** that were omitted in class.
- It does this using a Q&A format.
- I will try and have it reviewed by someone who teaches this material however I will be using a LLM pending this review so this is my best effort.
- I developed a number of misconception during the last two courses in the Bayesian specialization and it took a while to find and correct them. Hopefully this can save you some grief and get you productive much sooner.

---

### Q1: What Books are there on NDLMs? {#sec-faq-books}
...
### What is the difference between DLMs and NDLMS? {#sec-faq-dlm-vs-ndlm}
...
In the NDLM with known variance $delta_t$ we get to do what I intimated in the previous questions.
...
### The Books and the professor talk about the polynomial model what is that?

I was confused about this and there are three good reasons to be!

1. The AR($p$) has a characteristic polynomial we learned about
2. The polynomial model is a sub-model that covers the trend which could be an AR($p$) if the data is stationary
3. The book talks about Taylor series approximation in a multiple regression model. This is a polynomial in which we can pick the number of order
...
Getting back on topic there are state space models that incorporate identifing and switching between different models. These are called **hidden Markov models** (HMMs) and they are a generalization of NDLMs. HMMs can be used to model time series with structural change points, but they are more complex and require more data to estimate the parameters.
...
NDLMS are multifaced like diamonds:
  ...
  - architecture:
    ...
    - they can also incorporate regressors
    - they can incorporate geographical 
...
### Q. What does polynomial mean in a polynomial DLM ?
