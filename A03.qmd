...
\index{Laplace Distribution|see{Double Exponential Distribution}} \index{Double Exponential Distribution}
...
Now consider $X = \sigma Z+\mu$ where $\sigma > 0$ and $\mu$ is any real constant. Then $E(X) = E(\sigma Z+\mu) = \sigma E(Z) + \mu = \sigma_0 + \mu = \mu$ and \$Var(X) = Var(\sigma\^2 Z + \mu) = \sigma\^2 Var(Z) + 0 = \sigma\^2 \cdot 1 = \sigma\^2 \$
...
\index{t-Distribution} \index{Student's t-Distribution|see{t-distribution}}
...
![William Sealy Gosset AKA Student](images/bio-William_Sealy_Gosset.jpg){#fig-bio-student .column-margin width="200px"}
...
-   **Weibull** \index{Weibull} $Y = X^{\frac{1}{\gamma}}$
-   **Rayleigh** \index{Rayleigh} $Y = \sqrt{\frac{2X}{\lambda}}$
-   **Gumbel** \index{Gumbel} $Y=\alpha - \gamma \log(\frac{X}{\lambda})$
...
\index{F-Distribution} \index{F-ratio} [F Distribution]{.column-margin} [F-ratio]{.column-margin} The F-distribution or F-ratio, arises frequently as the null distribution of a test statistic, in the analysis of variance (ANOVA) and other F-tests.
