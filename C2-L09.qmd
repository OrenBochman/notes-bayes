...
When two covariates are highly correlated we call this relation **collinearity**. \index{collinearity} When one covariate in a multiple regression model can be linearly predicted from the others with a substantial degree of accuracy we call this relation **multicollinearity**. It is possible that no two pairs of a multicollinear \index{multicollinearity} group of covariates are correlated.
