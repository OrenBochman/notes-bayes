...
Unfortunately the course was missing the video with the derivation of the posterior 
from the hierarchical model of the location mixture of $AR(p)$ models. However I 
have endeavoured to fill this in using based on the final reading covering the more
sophisticated location and scale mixture of $AR(p)$.
...
Introducing latent configuration variable 

$$
L_t \in \{1,2,\cdots,K\} \qquad L_t=k \iff y_t\sim N(\mathbf{f}^T_t\boldsymbol{\beta}_k,\nu)
$$ {#eq-capstone-lm-Lt}
...
![full posterior distribution](images/C5-L3-SL08.png){#fig-capstone-lm-formulas .column-margin group="slides" width="53mm"}
...
That is, $p=2$ and $K=3$. 
Firstly, we set up the model by choosing prior hyperparameters. 
We use weakly informative prior for all parameters. 
That is, we set :
