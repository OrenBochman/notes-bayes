...
$$
\begin{split}
Pr(\omega,\beta,L,\nu|y) &\propto \prod_{k=1}^K  \\
   & \prod_{t:L_t=k} \mathcal{N}(y_t|f_t^\top \beta_k,L,\nu)  \\
   & \prod_{k=1}^K \omega_k^{\sum_{t=1}^T \mathbb{I}_{(L_t=k)}} \\
   & \prod_{k=1}^K \mathcal{N}(\beta\mid m_\nu) \mathcal{IG}(\nu|a_\nu,b_\nu)
\end{split}
...
$$
\omega| \cdots \sim Dir(a_i + \sum_{t=1}^T \mathbb{I}_{(L_t=i)}, \cdots, \sum_{t=1}^T \mathbb{I}_{(L_t=K)}) \qquad k=1,\cdots,K
$$
