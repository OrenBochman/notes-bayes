...
Consider again the coin-flipping example from the lesson. Recall that the posterior PDF for θ, after observing $Y=1$, was $f(\theta \mid Y=1) = 2\theta I_{\{0 \le \theta \le 1 \}}$. Which of the following is a correct interpretation of $\mathbb{P}r(0.3 < \theta < 0.9 \mid Y=1) = \int_{0.3}^{0.9} 2\theta d\theta = 0.72$?
:::
...
-   [ ] 50% interval: $θ\in(0.500,1.000)$
-   [ ] 50% interval: $θ\in(0.400,0.756)$
-   [ ] 50% interval: $θ\in(0.196,0.567)$
-   [x] 50% interval: $θ\in(0.326,0.674)$
