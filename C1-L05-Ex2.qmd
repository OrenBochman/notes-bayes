...
## Homework on Bayesian Inference {#sec-homework-on-bayesian-inference}
...
Consider again the coin-flipping example from the lesson. Recall that we used a $\text{Uniform}(0,1)$ prior for $θ$.
...
-   [x] $(0.3, 0.9)$ is a 60% credible interval for $θ$ before observing any data.
...
Consider again the coin-flipping example from the lesson. Recall that the posterior PDF for θ, after observing $Y=1$, was $f(\theta \mid Y=1) = 2\theta I_{\{0 \le \theta \le 1 \}}$. Which of the following is a correct interpretation of $P(0.3 < \theta < 0.9 \mid Y=1) = \int_{0.3}^{0.9} 2\theta d\theta = 0.72$?
:::
...
-   [x] $(0.3, 0.9)$ is a 72% credible interval for $θ$ after observing $Y=1$.
