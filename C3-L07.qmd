...
while(!sw){
  ...
  for(i in 1:(n+m)){
    for(k in 1:KK){
      KLn = KLn + v[i,k]*(log(w[k]) + mvtnorm::dmvnorm(x[i,],mu[k,],Sigma[k,,],log=TRUE)) # <13>
    }
  }
...
```{r}
#| label: lbl-em-classification-prediction-qda
...
```{r}
#| label: lbl-em-classification-prediction-lda
# lda
