...
The first example shows how we can use bayesian methods for spatial interpolation. The original idea by  [Danie G. Krige](https://en.wikipedia.org/wiki/Danie_G._Krige), was to dig holes in the ground and measure the amount of gold in the soil with the goal to then extrapolate the amount of gold in the soil in other locations and then decide where to dig next.
This method is called kriging and is widely used in geostatistics and spatial statistics. 

<!--
::: {#fig-nonparametrics-gps_2 .column-margin  group="slides" width="53mm"}
![why GP2](/images/c6/C6-SL02.png)
...
::: {#exm-1-krigging}
### Geospatial prediction for Oil and Gold deposits : Kriging for spatial interpolation

$$
\begin{aligned}
Y(\mathcal{X}) | D_N &\sim \mathcal{N}_{n'}(\mu(\mathcal{X}), \Sigma(\mathcal{X})) \\
\text{mean } \mu(\mathcal{X}) &= \Sigma(\mathcal{X}, X_N)\Sigma^{-1}_{N} Y_N \\
\text{variance } \Sigma(\mathcal{X}) &= \Sigma(\mathcal{X}, \mathcal{X}) - \Sigma(\mathcal{X}, X_N)\Sigma^{-1}_N\Sigma(\mathcal{X},X_N)\Sigma(\mathcal{X},X_N)^\top
\end{aligned}
$$ {#eq-kriging-prediction}

where $Y_N$ are the observed values at locations $X_N$ and $\Sigma(\cdot, \cdot)$ is the covariance function of the Gaussian process with dimensions $n' \times n$

This has some nice properties:
- The describe the best (minimising MSPE) linear unbiased predictor (BLUP) of the value at $\mathcal{X}$ given the data $D_N$.
- The prediction is a Gaussian distribution with a mean and variance that can be computed in closed form.
- The mean is a linear combination of the observed values, which means that the prediction is a weighted average of the observed values.
- The variance is a measure of uncertainty in the prediction, which can be used to guide the next steps in the exploration process (e.g., where to dig next).
:::



::: {#exm-2-ocean-currents}
### Ocean current where the velocity vector field varies by space & time
...
This example is taken from [@berlinghieri2023gaussianprocesseshelmholtzfluid];
 which extended earlier work in [@gonccalves2019reconstruction] and [@lodise2020investigating]. The idea is simple enough but the math of fluid dynamics is pretty complicated to begin with and I was intrigued how bayesian nonparametric methods could be used to solve it.

:::: {#fig-nonparametrics-gps_1 .column-margin  group="slides" width="53mm"}
![wbuoy trajectories and ocen current](/images/c6/C6-SL01.png)
Drifting buoy trajectories in the Gulf of Mexico superimposed on surface currents c.f. [Ryan, Özgökmen 2023] [@zewe2023betterway] 
::::
...
$$
\underbrace{\mathbf{F}}_{\text{ocean  flow}} = \underbrace{\nabla \Phi}_{divergent\  velocity} + \underbrace{\nabla \times \mathbf{\Psi}}_{rotational\  velocity}
...
\end{bmatrix}
$$ {#eq-helmholtz-decomposition-components} 


$$
\delta := \nabla \cdot \mathbf{F} = \frac{\partial F_x}{\partial x} + \frac{\partial F_y}{\partial y} = \frac{\partial^2 \Phi}{\partial x^2} + \frac{\partial^2 \Phi}{\partial y^2}
$$ {#eq-divergence-in-helmholtz-decomposition}

Note: I replaced the *pesky superscripts* with x and y. 
However the original notation may be simpler to generalize and more amenable to conversion into code. Also subscripts tend to be used to index data point in ML/ these papers 

$$
\zeta := \nabla \times \mathbf{F} = \frac{\partial F_y}{\partial x} - \frac{\partial F_x}{\partial y} = \frac{\partial^2 \Psi}{\partial y^2} + \frac{\partial^2 \Psi}{\partial x^2} 
$$ {#eq-curl-in-helmholtz-decomposition}

Bayesian Approach and Gaussian Process Prior.

In this paper the authors used a Bayesian approach to inferring $F$. 
They assumes a likelihood, for a noise model, relating the observed buoy velocities to the field F.

$$
y_m = F(x_m) + \epsilon_m, \quad \epsilon_m \stackrel{\text{ind}}{\sim} \mathcal{N}(0, \sigma^2_{\text{obs}} I_2), \quad 1 \le m \le M.
$$ {#eq-likelihood-helmholtz-decomposition}

for some $\sigma^2_{\text{obs}} > 0$ and independent noise across observations.

with standard spatiotemporal kernels (e.g., squared exponential kernels).
::::

**Challenges**

Data is sparse, expensive to collect and is not on a grid.
Currents are highly nonlinear but smooth in space-time.
We want uncertainty quantification.  $\frac{abc}{}$

A common theme I noticed in BNP is that MCMC is great for complex simulations. Once real data is available it turns out to be hard to integrate into the model.  
This is where the BNP models start to shine. They allow smart tweaks that are still tractable but allow more principled integration of the data into the model.

The authors used simulation (think parameter recovery study) where they created datasets where the data (currents and divergence) are fully specified yet the GP still failed to connect currents when the buoys are sparse and that the recovered divergence.

Actually this reminds me of a theorem in complex analysis where it is shown that one can reconstruct a holomorphic function from its values on a curve or even from a sequence of points that have a limit point in the domain. This seems to be a similar phenomenon where the function is smooth enough that we can reconstruct it from sparse data...
...
![posterior](/images/c6/C6-SL04.png){#fig-nonparametics-posterior .column-margin group="slides" width="53mm"}

![joint](/images/c6/C6-SL05.png){#fig-nonparametics-joint .column-margin group="slides" width="53mm"}
...
![](/images/c6/C6-SL06.png){#fig-nonparametrics-multivariate-gaussian-locations .column-margin group="slides" width="53mm"}

- $M =2$ bivariate Gaussian $[y^{(1)} y^{(2)}] \sim \mathcal{N}(\mathbf{\mu}, \mathbf{\Sigma})$
...
- Next: Use a Similar setup but an M-long Gaussian instead of just a bivariate
  ...
  - We generate $[y^{(1)}, \ldots,y^{(m)}]^\top \sim \mathcal{N}(\mathbf{\mu}, \mathbf{K})$
  - with
    $\mu=0_M$ and 
...
![](/images/c6/C6-SL05.png){#fig-nonparametrics-dimension-data .column-margin group="slides" width="53mm"}


![](/images/c6/C6-SL07.png){#fig-nonparametrics-dimension-item .column-margin group="slides" width="53mm"}
...
![histogram of squared inter-point distances](/images/c6/C6-SL09.png){#fig-multiple-inputs .column-margin group="slides" width="53mm"}
...
::: {#vid-demo-01-gp-draws-1 .column-margin group="slides"}
{{< video "https://www.youtube.com/watch?v=UJ1L0qjPCLQ" >}}
Robert B. Gramacy - A surrogate modeling journey through Gaussian processes - Industrial Statistics Section of ISBA
:::

::: {#vid-demo-01-gp-draws-1 .column-margin group="slides"}
{{< video "https://www.youtube.com/watch?v=i7aApzaO7j4" >}}
https://youtu.be/i7aApzaO7j4
:::

::: {.callout-tip}
## What's next?

Boderick's Workshop has give us a solid foundation in Gaussian processes. 

1. Different **kernels** from [@rasmussen2006gaussian]
1. **Surrogate modeling** based on the book by Robert B. Gramacy and the talk on the side
1. Dive Deeper into [@gramacy2020surrogates]
1. Bayesian Optimization - though I think this will be the subject of another small course....
