...
In fact, if the data are independent and identically distributed from a $Bernoulli(p)$, $Poisson(λ)$, or $Normal(μ, σ^2)$, then $\bar x$ is the MLE for $p$, $λ$, and $μ$ respectively.
