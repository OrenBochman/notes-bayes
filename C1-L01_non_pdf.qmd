...
$$\Omega \in \mathcal{F}$$ {#eq-sigma1}

2.  $\mathcal{F}$ is closed under complementation:

$$\forall A \in \mathcal{F} \implies A^c \in \mathcal{F}$$ {#eq-sigma2}

3.  $\mathcal{F}$ is closed under countable unions:

$$\exists A_i \in \mathcal{F} i=1,2,3... \implies \bigcup_{n=1}^\infty {A_i} \in \mathcal{F}$$ {#eq-sigma3} is called a $\sigma$-algebra or a $\sigma$-field .
:::
...
if $\Omega$ is a *sample space* (@def-sample-space) and $\mathcal{F}$ a $\sigma$-algebra (@def-sigma-algebra) for $\Omega$ then a function $P: \mathcal{f} \to [0,1]$ with the following properties 1. $$P(\Omega)=1$$ 2. countable additive for pairwise disjoint countable collections of events: is called a probability measure over $\mathcal{F}$. $$ \forall E_{ i \in \mathbb{N} } \quad   P(\bigcup_{n\in \mathbb{N} }{E_n})=\sum_{n\in \mathbb{N} } P(E_n)$$ {#eq-p-measure-addativity} then $P$ is a **probability measure** over $\mathcal{F}$
:::
...
$$ P(\emptyset) = 0 $$ {#eq-axiom-p1} Probabilities of all possible events (the space of all possible outcomes) must sum to one.

$$ P(\Omega) = 1 $$ {#eq-axiom-p2} $$A\cap B = \emptyset \implies P(A \cup B) = P(A)+P(B) $$ {#eq-axiom-p3}

::: {#trm-complements}
$$P(A^c) =1-P(A) \qquad \forall A\in\Omega $$ {#eq-complements}
:::
...
$$1 \ge P(X) \ge 0 \qquad \forall X\in \Omega \tag{probability axiom 1}$$

Probabilities add to one

$$\sum_{i\in \Omega} P(X=i)=1 \tag{probability axiom 2}$$ The complement of an event $A$ is $A^c$\

$$ P(S) = 1 $$ {#eq-probability-axiom-3}

If events $A_\lambda$ are mutually exclusive (only one event may happen):

$$P(A_1 \cup A_2) = P(A_1) + P(A_2) - P(A_1\cap A_1)$$ {#eq-probability-union-finite}

$$P(\bigcup_{\lambda\in \Omega} A_\lambda)=\sum_{\lambda \in \Omega} P(A_\lambda)$$ {#eq-probability-union}

if ${B_i}$ is a finite or countably infinite partition of a sample space then

$$P(A) = {\sum_{i=1}^{N} P(A \cup B_i)}= {\sum_{i=1}^{N} P(A|B_i)P(B_i)}$$ {#eq-law-of-total-probability}
...
![Bruno de Finetti](images/bio-Bruno-de-Finetti.jpg){.column-margin}
...
-   A1 be the event: Atacama is less than 10,000 $km^2$.
-   A2 be the event: Atacama is between 10,000 and 50,000 $km^2$
-   A3 be the event: Atacama is between 50,000 and 100,000 $km^2$
-   A4 be the event: Atacama is more than 100,000 $km^2$ Assign probabilities to A1...A4
...
| A1    | 0          | 10K        | $\frac{1}{4}$ |
| A2    | 10k        | 50K        | $\frac{1}{4}$ |
| A3    | 50k        | 100K       | $\frac{1}{4}$ |
| A4    | 100k       |            | $\frac{1}{4}$ |
:::
:::

w at this point? - The expected area for the region is $\frac{750,000}{15}=50,000 km^2$. - What Do I believe? - I believe that the administrative regions have significantly different sizes - from my familiarity with some other countries. - As I don't know if Atacama is large or small my best *bet* is to assign equal probabilities to each event.
...
|-------|------------|------------|----------------|
| A1    |            | 10K        | $\frac{1}{16}$ |
| A2    | 10k        | 50K        | $\frac{3}{16}$ |
| A3    | 50k        | 100K       | $\frac{6}{16}$ |
| A4    | 100k       |            | $\frac{6}{16}$ |
:::
:::
...
-   How do I revise my guesstimate?
    -   If the region sizes are equally sized I should gamble mostly on A2 and A3.
    ...
    -   A1 seems extremely unlikely as it necessitates the top three regions account for almost all of the area of the country. $\frac{750,000 - 14 * 10,000}{3} = 203,333.3$ that's about 4 times the average for each state.
    -   A2 is fairly unlikely to require the to p three regions to account for $\frac{(750,000-14*20000)}{3}=170,000$ each that's more than 3 times the average.
...
$$
\mathbb{V}ar[X]=\int_{-\infty}^{\infty} (x_i-\mu)^2 f(x)dx
...
$$
\begin{aligned}
\mathbb{C}ov[X,Y] &= \mathbb{E}[(X-\mathbb{E}[X])(Y-\mathbb{E}[Y])] \\&= \mathbb{E}_{X,Y}[XY]-\mathbb{E}_X[X]\mathbb{E}_Y[Y]
\end{aligned}
$$ {#eq-covariance}
