...
- Understand the basics of MCMC including @alg-metropolis-hastings and Gibbs sampling.
...
::: {#fig-c2-l04-MCMC-MH1}
![Metropolis-Hastings algorithm where the proposals come from a uniform prior distribution, to sample a normal one-dimensional posterior probability distribution.](/images/c2/MCMC-MH1.png){.column-margin group="slides" width="53mm"}

[@lee2015metamodel]
:::

::: {#fig-c2-l04-MCMC-MH1}
![Three markov chains converging to the target distribution.](/images/c2/MCMC-MH2.png){#fig-mcmc .column-margin group="slides" width="53mm"}

Each chain in blue, green, and yellow starts isolated on the RHS. The chains mix on the upper right, and the later points in red are kept while the rest (burn-in) are discarded. [credit](https://en.wikipedia.org/wiki/User:Vzografos]
:::

\index{Metropolis-Hastings}
The [Metropolis-Hastings](https://en.wikipedia.org/wiki/Metropolis%E2%80%93Hastings_algorithm) is an MCMC algorithm used to sample from a likelihood function (called the target distribution), even if we do not know its normalizing constant. 
To do this, we construct and sample from a Markov chain whose stationary distribution is the target distribution. 
We begin by selecting an arbitrary starting value and iteratively accepting or rejecting candidate samples drawn from another distribution, one that is easy to sample.
...
Let us say we wish to produce samples from a *target distribution* $\mathbb{P}r(\theta) \propto g(\theta)$, where we don't know the normalizing constant (since $\int g(\theta)d\theta$ is hard or impossible to compute). 
Thus, we are forced to work with the *unnormalized joint probability* $g(\theta)$. 
...
\begin{algorithmic}
\Procedure{MetropolisHastings}{$\theta_0, m$}
  \State Initialize $\theta_0$
  \For{$i = 1$ \To $m$}
    \State Draw a candidate sample $\theta^*$ from a proposal distribution $q(\theta^* \mid \theta_{i-1})$
    \State Compute the acceptance ratio $\alpha = \frac{g(\theta^*)q(\theta_{i-1} \mid \theta^*)}{g(\theta_{i-1})q(\theta^* \mid \theta_{i-1})}$
    \If{$\alpha \ge 1$}
      \State Accept $\theta^*$ and set $\theta_i = \theta^*$
    \Else
      \State Accept $\theta^*$ with probability $\alpha$, otherwise set $\theta_i = \theta_{i-1}$
    \EndIf
  \EndFor
\EndProcedure
\end{algorithmic}
...
Q. What is going on in lines 5-10 of the algorithm?

Lines 5-10 act as a **correction** [**correction**]{.column-margin} since the *proposal distribution* is not the *target distribution*. 
...
If the proposed move to the candidate is "advantageous," $(\alpha \ge 1)$ we "move" there and if it is *"disadvantageous,"* we still might move there, but only with probability $\alpha$. 

Because our decision to "move" to the candidate depends only on the current chain location (value), this is a *Markov chain*.

Question: is the acceptance ratio related to Importance Sampling? 

Yes, the acceptance ratio $\alpha$ can be seen as a correction factor similar to the one used in importance sampling. In importance sampling, we also have a proposal distribution and we need to correct for the fact that we are not sampling directly from the target distribution. The acceptance ratio in Metropolis-Hastings serves a similar purpose by correcting for the discrepancy between the proposal distribution and the target distribution, ensuring that the Markov chain converges to the correct stationary distribution.
:::
...
So under the Metropolis-Hastings algorithm, step one is we start at an arbitrary location. 
