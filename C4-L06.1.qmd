...
Each component DLM has both an observational noise and a system noise. We really only want to deal with one of each and even that will prove challenging. 
So if we add components, we need to customize the variances. 

A second bit that's tricky is the model's dimensions and how many of those are free variables.
Filtering gives us estimates of the parameters we call put into the $\theta$ but we will also need to infer the two variances. 
...
c.f. [@harvey1990forecasting] [@DurbinKoopman2012TSABSS §11.5][@west2013bayesian Ch.11])
:::
