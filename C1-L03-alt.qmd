...
$$ X \sim Bin[n,p]$$ {#eq-binomial-rv}

the probability function

$$f(X=x \mid \theta) = {n \choose x} \theta^x(1-\theta)^{n-x}$$ {#eq-binomial-pmf}

$$L(\theta)=\prod_{i=1}^{n} {n\choose x_i}  \theta ^ {x_i} (1− \theta) ^ {(n−x_i)}$$ {#eq-binomial-likelihood}

$$\begin{aligned}\ell( \theta) &= \log \mathcal{L}( \theta) \\&= \sum_{i=1}^n \left[\log {n\choose x_i} + x_i \log  \theta + (n-x_i)\log (1- \theta) \right].\end{aligned}$$ {#eq-binomial-log-likelihood}

$$\mathbb{E}[X]= N \times  \theta $$ {#eq-binomial-expectation}

$$\mathbb{V}ar[X]=N \cdot \theta \cdot (1-\theta)$$ {#eq-binomial-variance}

$$\mathbb{H}(X) = \frac{1}{2}\log_2 \left (2\pi n \theta(1 - \theta)\right) + O(\frac{1}{n})$$ {#eq-binomial-entropy}

$$\mathcal{I}(\theta)=\frac{n}{ \theta \cdot (1- \theta)}$$ {#eq-binomial-information}
