...
Clearly, the expression in the bottom of @eq-normalizing-constants does not integrate to 1 (it integrates to $\sqrt{2πs^2}$ ). Although it is not a PDF, it is proportional to the $N(m, s^2)$ PDF and can be normalized to represent the $N(m, s^2)$ distribution only. Likewise, the posterior $f(\theta\mid x)$ maintains its uniqueness as long as we specify it up to a proportionality constant.
