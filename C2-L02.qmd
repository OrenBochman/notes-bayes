...
The next level that we need is the prior distribution from $\mu$ and $\sigma^2$. For now we're going to say that they're independent priors. So that our prior from $\mu$ and $\sigma^2$ is going to be able to factor Into the product of two independent priors. [$p(\mu,\sigma^2)~=~p(\mu)p(\sigma^2)\ (independence)$]{.column-margin} We can assume independents in the prior and still get dependents in the posterior distribution.
