...
::: {#exr-gs-1}
[Gibbs]{.colum-margin} Which of the following descriptions matches the process of Gibbs sampling for multiple random variables?
:::
...
::: {#exr-gs-2}
[Gibbs]{.colum-margin} Suppose we have a joint probability distribution for four variables, $p(w,x,y,z)$ . Which of the following expresses the full conditional distribution for variable x?
:::
...
::: {#exr-gs-3}
[Gibbs]{.colum-margin} Suppose we have the following joint distribution for x,y, and z:
...
-   [x] $p(z \mid x, y) \propto e^{-5z} \frac{\Gamma(z+3)}{\Gamma(z)} y^{z-1} I_{z\ge0}$
...
::: {#exr-gs-4}
[Gibbs]{.colum-margin} The full conditional distribution in Question 3 is not a standard distribution that we can easily sample. Fortunately, it turns out that we can do a Metropolis-Hastings step inside our Gibbs sampler step for z.
...
-   [X] Draw $y_i$ from the full conditional $p(y \mid x,z)$, plugging in the value $z_i$ just drawn in step 1 for $z$.
...
::: {#exr-gs-5}
[Gibbs]{.colum-margin} Suppose we have a joint probability distribution for three variables: $p(x,y,z)$ . Identify the algorithm to perform Gibbs sampling for all three variables. 1 point
:::
...
::: {#exr-gs-6}
[Gibbs]{.colum-margin} In our model with normal likelihood and unknown mean $\mu$ and unknown variance $\sigma^2$ , we chose a normal prior for the mean and an inverse-gamma prior for the variance.
...
::: {#exr-gs-7}
[Gibbs]{.colum-margin} Suppose we repeat the analysis for n=6 companies in another industry and the data are:
