...
\mathbb{P}r(y_{1:T}\mid \phi_1, \ldots, \phi_p, \nu ) 
 ...
 &= \mathcal{N}(F^\top \mathbf{\phi},\nu\mathbb{I}_n) \\
\end{aligned}
$$ {#eq-capstone-ar-likelihood}

-   where
    -   $\mathbf{\phi}=(\phi_1,\ldots,\phi_p)^\top$ is the vector of parameters, and
...
$$
y = F^\top \mathbf{\phi} + \varepsilon \qquad \varepsilon \sim \mathcal{N}(0,\nu\mathbb{I}_n)
$$

$$
\mathbb{P}r(y \mid \mathbf{\phi},\nu) = \mathcal{N}(F^\top \mathbf{\phi},\nu\mathbb{I}_n)
$$ 

$$
\mathbf{y}\sim \mathcal{N}(\mathbf{F}^\top \mathbf{\phi},\nu\mathbf{I}_n),\quad \mathbf{\phi}\sim \mathcal{N}(\mathbf{m}_0,\nu\mathbf{C}_0),\quad \nu\sim IG(\frac{n_0}{2},\frac{d_0}{2}) 
...
$$
\mathbb{P}r(\mathbf{x})=(2\pi)^{-k/2}|\mathbf{\Sigma}|^{-1/2}\exp[-\frac{1}{2}(\mathbf{x}-\mathbf{\mu})^T\mathbf{\Sigma}^{-1}(\mathbf{x}-\mathbf{\mu})]
$$ {#eq-multivariate-normal-pdf}

where $\mathbf{x}$ is a $k$-dimensional vector, $|\mathbf{\Sigma}|$ is the determinant of the covariance matrix, and $\mathbf{\Sigma}^{-1}$ is the inverse of the covariance matrix.
...
A random vector $x$ of dimension $k$ follows a multivariate Student-t distribution with $\nu$ degrees of freedom, location $\mathbf{\mu}$, and scale matrix $\mathbf{\Sigma}$, denoted as $x \sim T_\nu(\mathbf{\mu}, \mathbf{\Sigma})$, if its density is given by:

$$
\mathbb{P}r(\mathbf{x})=\frac{\Gamma(\frac{\nu+k}{2})}{\Gamma(\frac{\nu}{2})(\nu\pi)^{k/2}}|\mathbf{\Sigma}|^{-1/2}[1+\frac{1}{\nu}(\mathbf{x}-\mathbf{\mu})^T\mathbf{\Sigma}^{-1}(\mathbf{x}-\mathbf{\mu})]^{-(\nu+k)/2}
$$
...
In this lesson, I present the derivation of the posterior distributions for $\mathbf{\phi}$ and $\nu$ for the following model:
...
-   $\mathbf{\phi}$ is the vector of parameters,
-   $\nu$ is the scale parameter,
-   $\mathbf{m}_0$ is the prior mean of $\mathbf{\phi}$,
-   $\mathbf{C}_0$ is the prior covariance matrix of $\mathbf{\phi}$,
-   $n_0$ and $d_0$ are the shape and scale parameters of the inverse-gamma prior for $\nu$.

i.e. we start with a Normal likelihood with a Normal prior for $\mathbf{\phi}$ and an inverse-gamma prior for $\nu$. Since we call the parameters $\phi$ and not $\theta$ it suggests that this isn't a typical linear regression but rather an AR($p$) model with
...
$$
\begin{aligned} 
\mathbb{P}r(\mathbf{\phi},\nu\mid\mathbf{y}) & \propto \mathbb{P}r(\mathbf{y} \mid \mathbf{\phi},\nu)\mathbb{p}r(\mathbf{\phi}\mid\nu)\mathbb{p}r(\nu)\\ 
&\propto \nu^{-n/2}\exp \left(-\frac{(\mathbf{y}-\mathbf{F}^T\mathbf{\phi})^T(\mathbf{y}-\mathbf{F}^T\mathbf{\phi})}{2\nu}\right)\\ 
&\quad \times \nu^{-p/2}\exp\left(-\frac{(\mathbf{\phi}-\mathbf{m}_0)^T\mathbf{C}_0^{-1}(\mathbf{\phi}-\mathbf{m}_0)}{2\nu}\right)\\ 
&\quad \times \nu^{-(\frac{n_0}{2}+1)}\exp\left(-\frac{d_0}{2\nu}\right)\\ 
&\propto \nu^{-(\frac{n^*}{2}+1)}\exp\left(-\frac{d^*}{2\nu}\right)\\ 
&\quad \times \nu^{-p/2}\exp\left(-\frac{(\mathbf{\phi}-\mathbf{m})^T\mathbf{C}^{-1}(\mathbf{\phi}-\mathbf{m})}{2\nu}\right)\\ 
&\propto \mathbb{P}r(\nu\mid\mathbf{y})\mathbb{p}r(\mathbf{\phi}\mid\nu,\mathbf{y}) 
\end{aligned}
...
Therefore, we have the posterior distribution of $\mathbf{\phi}$ and $\nu$ as:

$$
\begin{aligned} 
\mathbb{P}r(\mathbf{\phi},\nu \mid \mathbf{y}) &=\mathbb{P}r(\mathbf{\phi} \mid \nu,\mathbf{y})\mathbb{P}r(\nu\mid \mathbf{y}) \\
&=\mathcal{N}(\mathbf{\phi}\mid \mathbf{m},\nu\mathbf{C})\ \mathcal{IG}(\nu\mid \frac{n^*}{2},\frac{d^*}{2}) \end{aligned}
$$ {#eq-posterior-distribution}

To get the $s$^th^ sample of $(\mathbf{\phi}^{(s)},\nu^{(s)})$ from the joint posterior distribution of $\mathbf{\phi}$ and $\nu$, we first sample $\nu^{(s)}$ from $\mathcal{IG}(\frac{n^*}{2},\frac{d^*}{2})$, then sample $\mathbf{\phi}^{(s)}$ from $\mathcal{N}(\mathbf{m},\nu^{(s)}\mathbf{C})$.

**One can also obtain posterior samples** of $\mathbf{\phi}$ by directly sampling from its **marginal posterior distribution.** Integrate out $\nu$ from the joint posterior distribution, we have

$$
\mathbb{P}r(\mathbf{\phi} \mid \mathbf{y})=T_{n^*}(\mathbf{m},\frac{d^*}{n^*}\mathbf{C})
$$

Denote the $s$^th^ posterior sample of $\mathbf{\phi}$ and $\nu$ as $\mathbf{\phi}^{(s)}$ and $\nu^{(s)}$, we can obtain the corresponding $i$^th^ posterior sample of $\mathbf{y}^{(s)}$ by sampling from $\mathcal{N}(\mathbf{F}^\top \mathbf{\phi}^{(s)},\nu^{(s)})$. Therefore, we can obtain the posterior point and interval estimates for the time series.
