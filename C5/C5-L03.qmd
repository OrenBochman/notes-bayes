...
$$
\begin{aligned} 
&y_t\sim\sum_{k=1}^K\omega_kN(\mathbf{f}^T_t\mathbf{\beta}_k,\nu),\quad \mathbf{f}^T_t=(y_{t-1},\cdots,y_{t-p})^T,\quad t=p+1,\cdots,T\\ 
&\omega_k\sim Dir(a_1,\cdots,a_k),\quad \mathbf{\beta}_k\sim \mathcal{N}(\mathbf{m}_0,\nu_k\mathbf{C}_0),\quad \nu \sim \mathcal{IG}(\frac{n_0}{2},\frac{d_0}{2}) 
\end{aligned}
$$ {#eq-capstone-loc-arp-mixture}

-   where:
    -   $\mathbf{f}_t$ is the design matrix of the AR model,
    -   $\mathbf{\beta}_k$ is the coefficient vector for the $k$-th component,
...
$$
\mathbf{\beta}=(\beta_1,\cdots,\beta_K) \qquad \mathbf{\omega}=(\omega_1,\cdots,\omega_K)
$$
...
$$
p(\mathbf{\beta},\mathbf{\nu},\mathbf{\omega},\mathbf{L}\mid\mathbf{Y},\mathbf{F}) \propto p(\mathbf{Y}\mid\mathbf{\beta},\mathbf{\nu},\mathbf{\omega},\mathbf{F})p(\mathbf{\beta})p(\mathbf{\nu})p(\mathbf{\omega})p(\mathbf{L})
...
$$ 
\begin{aligned} 
& \mathbb{P}r(\mathbf{\omega},\mathbf{\beta},\nu,\mathbf{L}|\mathbf{y},\mathbf{F}) \\
&\qquad \propto \mathbb{P}r(\mathbf{y}|\mathbf{\omega},\mathbf{\beta},\nu,\mathbf{L}) \\
&\qquad\qquad \mathbb{P}r(\mathbf{L}|\mathbf{\omega}) \\
&\qquad\qquad \mathbb{P}r(\mathbf{\omega}) \\
&\qquad\qquad \mathbb{P}r(\mathbf{\beta}) \\
&\qquad\qquad \mathbb{P}r(\nu)\\ 
&\qquad \propto \prod_{k=1}^K\prod_{\{t:L_t=k\}}N(y_t \mid \mathbf{f}^\top_t\mathbf{\beta}_k,\nu)\\
&\qquad \qquad \prod_{k=1}^K\omega_k^{\sum_{t=1}^T\mathbb{I}{(L_t=k)}} \quad \prod_{k=1}^K\omega_k^{a_k-1} \\
&\qquad\qquad\prod_{k=1}^K\left(\mathcal{N}(\mathbf{\beta}_k \mid \mathbf{m}_0,\mathbf{C}_0) \ \mathcal{IG}(\nu\mid\frac{n_0}{2},\frac{d_0}{2})\right) 
\end{aligned} 
...
&\omega_k\sim Dir(a_1,\cdots,a_k),\quad \mathbf{\beta}_k\sim N(\mathbf{m}_0,\nu_k\mathbf{C}_0),\quad \nu_k\sim IG(\frac{n_0}{2},\frac{d_0}{2}) 
\end{aligned}
...
$$
p(\mathbf{\beta},\mathbf{\nu},\mathbf{\omega},\mathbf{L}|\mathbf{Y},\mathbf{F}) \propto p(\mathbf{Y}|\mathbf{\beta},\mathbf{\nu},\mathbf{\omega},\mathbf{F})p(\mathbf{\beta})p(\mathbf{\nu})p(\mathbf{\omega})p(\mathbf{L})
$$
...
$$ 
\begin{aligned} 
p(\mathbf{\omega},\mathbf{\beta},\nu,\mathbf{L}|\mathbf{y})&\propto p(\mathbf{y}|\mathbf{\omega},\mathbf{\beta},\nu,\mathbf{L})p(\mathbf{L}|\mathbf{\omega})p(\mathbf{\omega})p(\mathbf{\beta})p(\nu)\\ 
&\propto \prod_{k=1}^K\prod_{\{t:L_t=k\}}N(y_t\mid\mathbf{f}^T_t\mathbf{\beta}_k,\nu)\prod_{k=1}^K\omega_k^{\sum_{t=1}^T\mathbf{1}(L_t=k)}\prod_{k=1}^K\omega_k^{a_k-1}\prod_{k=1}^K(N(\mathbf{\beta}_k\mid\mathbf{m}_0,\mathbf{C}_0)IG(\nu_k|\frac{n_0}{2},\frac{d_0}{2})) 
\end{aligned} 
$$

1.For $\mathbf{\omega},$ we have :

$$
\mathbf{\omega}\mid \cdots\sim Dir(a_1+\sum_{t=1}^T\mathbf{1}(L_t=1),\cdots,a_K+\sum_{t=1}^T\mathbf{1}(L_t=K)) 
$$

2.  For $L_t$ we have

$$
p(L_t=k\mid\cdots)\propto \omega_kN(y_t\mid\mathbf{f}^T_t\mathbf{\beta}_k,\nu) 
$$
...
3.  For $ν_k$ and $\mathbf{\beta}_k$ denote $\mathbf{\tilde{y}}_k:= \{y_t: L_t=k\}$ and $\mathbf{\tilde{F}}_k$ as the design matrix belonging to $\mathbf{\tilde{y}}_k$, and $n_l=\sum_{}^{T}\mathbb{1}(L_t=k)$, we have $\nu_k\mid \cdots \sim \mathcal{IG}(\frac{n_k^*}{2},\frac{d_k^*}{2})$ and $\mathbf{\beta}_k \sim \mathcal{N}(\mathbf{m}_k,\nu_k\mathbf{C}_k)$ , where 
