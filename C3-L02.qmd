...
$$
\mathbf{x} = x_1, \ldots, x_n \implies \text{the collected observations}
$$ {#eq-observed-data}

The observation $\mathbf{x}$, are independent and identically distributed (i.i.d.) samples from the mixture distribution. We model them using a mixture as we have seen before in @eq-finite-mixture-cdf-hierarchical-2.

$$
\mathbf{x} \sim f(\mathbf{x} ) = \sum_{k=1}^K w_k \cdot g_k(x_i \mid \theta)
