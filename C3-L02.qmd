...
-   We can construct two different types of likelihood function:
    ...
    2.  the **complete data likelihood** is the probability of observing the data and the latent variables given the parameters of the model. discuss what's called the complete data likelihood that relies on the hierarchical representation that we discussed in the previous lecture
...
Our assumption is that we have a set of observations $\mathbf{x} = (x_1, \ldots, x_n)$, which are independent and identically distributed (i.i.d.) samples from the mixture distribution. We can write this as:

We model them using a mixture as we have seen before in @eq-mixture-marginal-cdf.
...
$$
\mathcal{L}(\boldsymbol{\omega},\boldsymbol{\theta} \mid  \mathbf{x}) = \prod_{i=1}^n \sum_{k=1}^K w_k \cdot g_k(x_i \mid \theta)
...
we will now introduce the latent variable $C_i$ which indicates which component generated the observation $x_i$.
$$
...
$$
\begin{split}
\mathcal{L}(\boldsymbol{\omega},\boldsymbol{\theta}, \mathbf{C}) &= \prod_{i=1}^n \prod_{k=1}^K [\omega_k g_k(x_i)]^{\mathbb{1}_{(C_i=k)}}  \\
&=
\prod_{i=1}^n \prod_{k=1}^K [ g_k(x_i)]^{\mathbb{1}_{(C_i=k)}} 
\prod_{i=1}^n \prod_{k=1}^K \omega_k ^{\mathbb{1}_{(C_i=k)}} \\
&=
\prod_{i=1}^n \prod_{k=1}^K [ g_k(x_i)]^{\mathbb{1}_{(C_i=k)}} 
\prod_{k=1}^K \omega_k ^{\sum_{i=1}^n \mathbb{1}_{(C_i=k)}}
\end{split}
$$

The last step is possible because the basis of this expression is the same for every $i$, allowing the right double product can also be written as a single product. 
If we think about what the sum represents, the sum just represents the number of observations in the sample that belong to up to component k. 
So that provides a very nice break, and a very nice interpretation for the likelihood function, and for a complete data likelihood function. 
You have one piece of the likelihood function that corresponds to the distribution of the observations.

Basically, if you know what component generated each observation, you just use the density of that component in this product. 
So this simplifies this expression substantially. You are going for each $i$. to retain one of the key terms that corresponds to the exponent that is equal to one. Then, you have a second term that corresponds to this product of the weights. In every case, the weight is going to be raised to essentially the number of observations, that belong to that component.



This allows us to write the complete data likelihood as a product of the likelihoods of the individual components, weighted by their respective weights.
