...
![MCMC - Priors of convenience](images/c3l3-ss-01-mcmc-part1.png){.column-margin}

![MCMC - Complete data Likelihood](images/c3l3-ss-02-mcmc-part1.png){.column-margin}
...
$$
p(w) = \frac{\Gamma(\sum_{k=1}^K \alpha_k)}{\prod_{k=1}^K \Gamma(\alpha_k)} \prod_{k=1}^K w_k^{\alpha_k - 1} \qquad \sum_{k=1}^K w_k = 1
$$ {#eq-dirichlet-prior}
...
$$
\begin{align*}
 p(\mathbf{x}, \mathbf{c}, \mid \mathbf{\omega}, \mathbf{\theta})  &= \prod_{i=1}^n \prod_{k=1}^K (\omega_k\ g_k(x_i \mid \theta_k))^{\mathbb{1}(c_i = k)} &
\\& = \left[\prod_{k=1}^K \prod_{\{i:c=k\}}^n g_k(x_i \mid \theta_k)\right] && \left [\prod_{k=1}^n \omega_{k}^{\sum \mathbb{1}(c_i = k)}\right ] 
\\& = p(\mathbf{x} \mid \mathbf{c}, \mathbf{\omega}, \mathbf{\theta}) && p(\mathbf{c} \mid \mathbf{\omega}, \mathbf{\theta}) 
\\& = p(\mathbf{x} \mid \mathbf{c}, \mathbf{\theta}) && p(\mathbf{c} \mid \mathbf{\omega}) 
\end{align*}
...
![posterior distribution - weights](images/c3l3-ss-03-mcmc-part2.png){.column-margin}

![posterior distribution - components](images/c3l3-ss-04-mcmc-part2.png){.column-margin}

![posterior distribution - parameters](images/c3l3-ss-05-mcmc-part2.png){.column-margin}
...
$$
p(c, \theta, \omega | x) \propto  \left \{ \prod_{i=1}^n p(x | c, \theta, \omega) \right \} \left \{ \prod_{i=1}^n\prod_{k=1}^K p(c | \omega, \theta) \right \}\ p(\omega)\ p(\theta)
...
Now that we have a clear structure for the likelihood function that we will be using and we have prior distributions for
all of our parameters, we can proceed to derive the posterior distribution that we will be working with. 
...
![location mixture of Normals - priors](images/c3l3-ss-06-mcmc-for-location-mixtures-of-normals-part-1.png){.column-margin}

![location mixture of Normals - marginals ](images/c3l3-ss-07-mcmc-for-location-mixtures-of-normals-part-1.png){.column-margin}

![location mixture of Normals weights](images/c3l3-ss-08-mcmc-for-location-mixtures-of-normals-part-1.png){.column-margin}

![location mixture of Normals - components](images/c3l3-ss-09-mcmc-for-location-mixtures-of-normals-part-1.png){.column-margin}

![location mixture of Normals - $\sigma^2$](images/c3l3-ss-10-mcmc-for-location-mixtures-of-normals-part-1.png){.column-margin}
...
![location mixture of Normals $\mu$](images/c3l3-ss-11-mcmc-for-location-mixtures-of-normals-part-2.png){.column-margin}

![location mixture of Normals $\mu$ continued 1](images/c3l3-ss-12-mcmc-for-location-mixtures-of-normals-part-2.png){.column-margin}

![location mixture of Normals $\mu$ continued 2](images/c3l3-ss-13-mcmc-for-location-mixtures-of-normals-part-2.png){.column-margin}
