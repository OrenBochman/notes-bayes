...
::: {.solution  .column-screen-inset-right  .column-screen-inset-right }

Here the shocks just shift means; 
the Kalman‐style recursions are unchanged except the prediction/forecast means include $\bar{w}_t,\bar{v}_t$.

We start by postulating the following constant NDLM model

$$
\begin{aligned}
\mu_t&=\mu_{t-1}+\omega_t, & \omega_t\sim\mathcal N(\bar w_t,W_t),\\
Y_t&=\mu_t+\nu_t, & \nu_t\sim\mathcal N(\bar v_t,V_t),\\
...
$$
\begin{aligned}
a_t &= m_{t-1} + \bar w_t & \text{state prior mean (evolution drift)}\\
R_t &= C_{t-1} + W_t     & \text{state prior var (evolution noise)}\\
f_t &= a_t + \bar v_t    & \text{1-step forecast mean (obs bias)}\\
Q_t &= R_t + V_t         & \text{1-step forecast var (obs noise)}\\
A_t &= \dfrac{R_t}{Q_t}  & \text{Kalman gain = prior/forecast var}\\
e_t &= y_t - f_t         & \text{innovation (forecast error)} \\
m_t &= a_t + A_t e_t          & \text{posterior mean update}\\
C_t &= R_t - A_t R_t=R_t-\frac{R_t^2}{Q_t} & \text{posterior var update}
\end{aligned}
$$
...
### (a) Via Bayes’ theorem (completing the square)
