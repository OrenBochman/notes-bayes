...
<!-- 
Suppose our client is running an online store and we are tasked in analyzing data the marketing and sales of multiple products in the capacity of a consulting data scientist with express goals of decision support. We may have either classification or regression tasks to perform on each product, e.g. allocate marketing budget, recommend the price, estimate self elasticity of demand, and cross elasticity of demand for related products i.e (complements and substitutes). But ultimately we need to recommend a marketing strategy together with and associated risk. 

One strategy might be to increase or decrease the price for a product. Another strategy might be to increase or decrease it advertising. A more complex strategy might be to modify both. Also if products are related we might make changes to multiple related products.

- If a product's demand is elastic one might expect to see a significant drop in demand in response to an increase in price. (Buyers have alternatives and are sensitive to their budget constraints.) 
- If a product's demand is inelastic, e.g. fuel or insulin one might expect to see a smaller drop in demand in response to an increase in price. (Buyers lack alternatives and are willing to sacrifice their other preferences to satisfy their budget constraints.)

For related products we might have more complex strategies:
- decrease the price of one related product and increase the price of another related product. 
- change the number of units sold for each product. (e.g. a bundle of four tuna can instead of three).
- adjust the marketing spend for each product.


We might do 
If the product is a complement, we might also consider adjusting the price of the related product.

1. $\delta advertising per media  )
2. \delta price | product)
based on the estimated elasticity together with 

When we run regression at scale we may have lots of data to deal with while at the same time we may actually have to deal with sparsity
-->

Sometimes we want to fit different regression lines for different groups
We may want the group to be definition to be either hard (one group only) or soft (wights for multiple groups).


TODO: make a model that does hard clustered regression 
TODO: make a model that does soft clustered regression 

Another point to consider is that some products like a Milk is on a consumed  on regular basis but a TV set is consumed only once the old one breaks down.

This suggest a [Bass diffusion model](https://en.wikipedia.org/wiki/Bass_diffusion_model) type model for the consumption of these products.
...
Here is where I can collect questions about TS analysis that I have as I go through the course. 
Hopefully I will be better equipped to answer many of these  them by the end of the course.

:::  

::: {.callout-tip}
## Overthinking Elasticity of Demand over Time

The usual definition of elasticity of demand is the percentage change in quantity demanded divided by the percentage change in price.

However in reality there is a time dimension to the problems that can  bring forth added complexity.

### Long term Short Term

1. there is a short term elasticity of demand.
2. there is a long term elasticity of demand.

E.g. fuel price is inelastic in the short term as people don't have alternatives in the short term. In the [long term high fuel prices] e.g. [1970 oil crisis](), [1979 oil crisis](), [2021-2023 energy crisis]() saw high oil prices for years which led to significant changes in consumption like car pooling, shift to fuel efficient and more recently hybrid and electric vehicles.

This suggest we are dealing with a time series with a different structure at different scales.

- Prof. Raquel Prado and the authors discusses something like this in [@prado2000bayesian] in the context of EEG, stating that wavelet and other orthogonal decompositions that have been found useful for analyzing non-stationary time series data fail to capture the  structure of the data at different scales.
   - TODO: look into the paper regarding the solution 
- Prof Herbert K. H. Lee and and the author discusses Multiscale Time Series in [@ferreira2007multiscale].

### Buffer Effect

An anecdotes about chicken/turkeys being sold as (a massive) loss leader before holiday meals prompted poorer population to invest in communal freezers and buy a year's worth supply. 

It is worth noting that expected shocks to price can prompt buffering behavior 
or while supply shocks can temporarily boost demand.

I imagine this effect could be modeled well using a MA($q$) process. 
- TODO: code a NDLM with this buffering....

### Seasonality

Many products have seasonal demand, Ice skates have high demand in winter and low demand otherwise (if there is a skating venue).

- We can model this with a TVAR($p$) process or more sophisticated markov switching Model.
