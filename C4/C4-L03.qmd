...
-   $\mathbf{\beta} = (\beta_1, \ldots, \beta_k)'$ is the k-dimensional vector of regression coefficients,
-   $\mathbf{\varepsilon} = (\varepsilon_1, \ldots, \varepsilon_n)'$ is the n-dimensional vector of errors,
-   $\mathbf{I}$ is an $n \times n$ identity matrix.

If $\mathbf{X}$ is a *full rank* matrix with rank $k$ , the maximum likelihood estimator for $\mathbf{\beta}$, denoted as $\hat{\mathbf{\beta}}_{MLE}$ is given by:

$$
\hat{\mathbf{\beta}}_{MLE} = (\mathbf{X}'\mathbf{X})^{−1}\mathbf{X}'\mathbf{y},
...
$$
\hat{v}_{MLE} = \frac{1}{n} (y − \mathbf{X} \hat{\mathbf{\beta}}_{MLE})′(y − \mathbf{X} \hat{\mathbf{\beta}}_{MLE})
$$ {#eq-mle-estimator-v}

$\hat{v}_{MLE}$ is not an unbiased estimator of $v$, therefore, the following unbiased estimator of $v$ is typically used:

$$
s^2 = \frac{1}{n-k}(y − \mathbf{X} \hat{\mathbf{\beta}}_{MLE} )′(y − \mathbf{X} \hat{\mathbf{\beta}}_{MLE} )
...
$$
\mathbb{P}r(y \mid \mathbf{\beta} , v) = \frac{1}{(2\pi v)^{n/2}}\exp \left\{ -\frac{1}{2} (y − \mathbf{X} \mathbf{\beta})′(y − \mathbf{X} \mathbf{\beta}) \right\}
$$

If a prior of the form :

$$ 
\mathbb{P}r(\mathbf{\beta}, v) \propto \frac{1}{v}
$$ {#eq-prior-regression}

is used, we obtain that the posterior distribution is given by:

$$
\mathbb{P}r(\mathbf{\beta},v \mid \mathbf{y}) \propto \frac{1}{v^{n/2+1}}\exp \left\{ -\frac{1}{2v} (\mathbf{y} − \mathbf{X} \mathbf{\beta})′(\mathbf{y} − \mathbf{X} \mathbf{\beta}) \right\}
$$ {#eq-posterior-regression}

In addition it can be shown that

-   $(\mathbf{\beta}\mid v, \mathbf{y}) \sim \mathcal{N} (\hat{\mathbf{\beta}}_{MLE} , v(\mathbf{X}'\mathbf{X})^{-1})$
-   $(v \mid \mathbf{y}) \sim \mathcal{IG}\left(\frac{(n − k)}{2}, \frac{1}{2}d\right)$ with

$$
d = (\mathbf{y} − \mathbf{X} \hat{\mathbf{\beta}}_{MLE} )′(\mathbf{y} − \mathbf{X} \hat{\mathbf{\beta}}_{MLE} )
...
with $k = dim(\mathbf{\beta})$.

[Given that $\mathbb{P}r(\mathbf{\beta}, v \mid \mathbf{y}) = \mathbb{P}r(\mathbf{\beta} \mid v, \mathbf{y})p(v \mid \mathbf{y})$ the equations above provide a way to directly sample from the posterior distribution of $\mathbf{\beta}$ and $v$ by first sampling v from the inverse-gamma distribution above and then conditioning on this sampled value of v, sampling $\mathbf{\beta}$ from the normal distribution above.]{.mark}
