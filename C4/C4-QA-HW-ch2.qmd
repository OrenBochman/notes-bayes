...
Discount weighted regression applied to a locally constant process estimates the current level at time $t$ as that value $M_t$ of $\mu$ that, given $Y_1, \ldots, Y_t$, minimizes the discounted sum of squares
...
$$
\mathbb{V}[e_t \mid \mathcal{D}_{t-1}] \to Q \text{ and } (Y_{t+1} - Y_t - e_{t+1} + \delta e_t) \to 0
$$
...
In the constant DLM $\{1, 1, V, W\}$ , show that $R_t = \frac{C_{t-1}}{\delta_t}$, where $\delta_t \in (0,1]$ . Thus, the constant DLM updating equations are equivalent to those in a discount DLM with discount factors $\delta_t$ changing over time. Find the limiting value of $\delta_t$ as $t$ increases, and verify that $\delta_t$ increases or decreases with $t$ according to whether the initial variance $C_0$ lies below or above the limiting value $C$ .
:::
...
variation is independent of $C_t$. What is this value when $V = 97$ and $W = 6$?

(b) Supposing that $C_t = C$, the limiting value, show that the 
corresponding value of $\frac{L_t(k)}{V}$ depends only on $k$ and $r = \frac{W}{V}$.
For each value of $r = 0.05$ and $r = 0.2$ , plot the ratio $\frac{L_t(k)}{V}$
...
To do this graph the distribution using an appropriate computer package and ﬁnd the upper 90%, 95%, 97.5% and 99% points of the $\mathcal{T}_n[0, 1]$ distribution for $n =2, 5, 10$ and $20$ degrees of freedom, comparing these with those of the $\mathcal{N}[0, 1]$ distribution.
...
autocorrelation coefficient of lag $k$, $C[Z_t, \frac{Z_{t+k}]}{V[Z_t]}$ , is
...
Assuming the model true, the prior marginal distribution of $r_k$, for every $|k| > 1$, is roughly $\mathcal{N}[0, \frac{1}{\sqrt{n}}]$. 
...
Note also that if $\frac{V_t}{W_t}$ is constant, the whole graph $\{r_k, k\}$ is expected to look exactly the same. 
...
$$
C(k) = \mathbb{C}[e_{t+k}, e_t] = \delta^{|k| - 1} \, Q^* \, \frac{(\delta - \delta^*)(1 - \delta \delta^*)}{1 - \delta^2}.
$$

Examine graphs of $\{\delta, \frac{Q}{Q^*}\}$ and of $\{\delta, \frac{C(1)}{Q}\}$ for the typical practical cases $\delta^* = 0.9, \delta^* = 0.8$ and for the atypical case $\delta^* = 0.5$.
:::
