...
with a hyperparameter like a polynomial degree $q$, an AR($p$) or the number of fourier components $r$ and we give it to the Kalman Filter, which returns an estimate for the $\mathbf{\theta}_t$ for us. But that still leaves $\nu_t$ and $\omega_t$. 
