...
|---------------|------------------|------|------------------|------------------------------------------------------------|---------|
| Normal        | $\mathbb{R}$     |      | Identity         | $\mathbf {X} {\mathbf {\beta }}=\mu$                   |         |
| Expoential    | $\mathbb{R}^+_0$ |      | Negative inverse | $\mathbf {X} {\mathbf {\beta }}=-\mu^{-1}$             |         |
| Gamma         | $\mathbb{R}^+_0$ |      | Negative inverse | $\mathbf {X} {\mathbf {\beta }}=-\mu^{-1}$             |         |
| Inverse Gamma | $\mathbb{R}^+_0$ |      | Inverse squared  | $\mathbf {X} {\mathbf {\beta }}=\mu^{-2}$              |         |
| Poisson       | $\mathbb{N}_0$   |      | Log              | $\mathbf {X} {\mathbf {\beta }}=ln(\mu)$               |         |
| Bernuolli     | $\{0,1\}$        |      | Logit            | $\mathbf {X} {\mathbf {\beta }}=ln(\frac{\mu}{1-\mu})$ |         |
| Binomial      |                  |      | Logit            | $\mathbf {X} {\mathbf {\beta }}=ln(\frac{\mu}{n-\mu})$ |         |
| Categorical   |                  |      | Logit            | $\mathbf {X} {\mathbf {\beta }}=ln(\frac{\mu}{1-\mu})$ |         |
| Multinomial   |                  |      | Logit            | $\mathbf {X} {\mathbf {\beta }}=ln(\frac{\mu}{1-\mu})$ |         |
