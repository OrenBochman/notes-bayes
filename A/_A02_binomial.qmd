...
|------------------------------------------------------------|------------------|
| $X \sim Bin[n,p]$| RV|
| $f(X=x \mid p) = {n \choose x} p^x(1-p)^{n-x}$ | PMF
| $F_X(k,p)= I_{q}(n-\lfloor k\rfloor ,1+\lfloor k\rfloor )$ |CDF|
| $\mathbb{E}[X]= n \times p$ | Exception |
| $\mathbb{V}ar[X]=n \times p \times (1-p)$ | Variance |
| $\mathbb{M}_X[t]=(q+pe^{t})^{n}$ | MGF |
| $\varphi_X(t) = (q+pe^{it})^{n}$ | Characteristic fn. |
| $\mathbb{H}(X)= \frac{1}{2}\log_2 \left (2\pi n p(1 - p)\right) + O(\frac{1}{n})$ | Entropy |
| $\mathbb{I}_X(p) = \frac{1}{p^{2}(1-p)}$ | Fisher Information |
| $\mathcal{L}(p)=\prod_{i=1}^{n} {n\choose x_i}p^{x_i} (1−p)^{(n−x_i)}$| Likelihood fn. |
| $\mathcal{l}(p)=\frac{n}{ p \times (1- p)}$| Log Likelihood fn. |
